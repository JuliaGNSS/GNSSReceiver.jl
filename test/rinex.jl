...
# GPS week 2124 (the week 10-bit `WN = 76` resolves to around mid-2020) starts on
...
_gps_lnav_data(; kwargs...) = GNSSDecoder.GPSL1CAData(;
    ...
    WN = 76,
    code_on_L2 = 1,
    # URA index 0, the 2.0 m the record below expects.
    URA_index = 0,
    sv_health = 0,
    IODC = 63,
...
_galileo_inav_data(; kwargs...) = GNSSDecoder.GalileoINAVData(;
    WN = _GPS_WEEK - 1024,
    TOW = 82596,
    SVID = 24,
    ...
    BGD_E1_E5a = 2.7939677238e-9,
    BGD_E1_E5b = 3.4924596548e-9,
    kwargs...,
)
...
@testset "GPS LNAV ephemeris record" begin
    ...
    eph_unhealthy = GNSSReceiver.rinex_ephemeris(
        _decoder_with(GPSL1CA(), 13, _gps_lnav_data(; sv_health = 0b111111));
        approximate_year = 2020,
    )
...
@testset "Galileo ephemeris records" begin
    ...
    e5b_down = _decoder_with(
        GalileoE1B(),
        24,
        _galileo_inav_data(; E5b_SHS = GNSSDecoder.signal_out_of_service),
    )
