...
function process(
    ...
    if receiver_state.runtime % acquire_every == 0ms
        ...
        new_sat_channel_states = Dict(
            res.prn => SatelliteChannelState(
                TrackingState(res; num_ants, post_corr_filter = N == 1 ? Tracking.DefaultPostCorrFilter() : EigenBeamformer(N)),
                res.prn in keys(sat_channel_states) ? sat_channel_states[res.prn].decoder : GNSSDecoderState(system, res.prn),
                CodeLockDetector(),
                CarrierLockDetector(),
                0ms
            )
    ...
    sat_channel_states = Dict(
        prn => is_in_lock(state) ? SatelliteChannelState(
            ...
            foldl((prev_detector, cn0) -> update(prev_detector, cn0), get_cn0.(track_results[prn]), init = sat_channel_states[prn].code_lock_detector),
            foldl((prev_detector, prompt) -> update(prev_detector, prompt), get_filtered_prompt.(track_results[prn]), init = sat_channel_states[prn].carrier_lock_detector),
            state.time_in_lock + signal_duration
        ) : state
        for (prn, state) in sat_channel_states
    )
    sat_states = [SatelliteState(sat_channel_states[prn].decoder, track_results[prn][end]) for prn in keys(filter(((prn, state),) -> is_in_lock(state) && state.time_in_lock > time_in_lock_before_pvt, sat_channel_states))]
    pvt = receiver_state.pvt
    if length(sat_states) > 3
        pvt = calc_pvt(sat_states, pvt)
    end
    ReceiverState(sat_channel_states, pvt, receiver_state.runtime + signal_duration), track_results
end
...
function track_measurement_parts(track_state, measurement, sampling_freq, signal_duration)
    ...
    first_track_result = track(view_part(measurement, 1:samples), track_state, sampling_freq)
    ...
    for i = 2:num_parts
        track_results[i] = track(view_part(measurement, (i - 1) * samples + 1:i * samples), track_state, sampling_freq)
