...
# The GPS time-scale origin, taken from the package that owns it rather than restated as a
# literal. Every RINEX epoch and ephemeris record is stamped in GNSS system time counted
# from it. The other two constants this file works in are already bound for the whole
# package (see the imports in `GNSSReceiver.jl`), in the units it needs them:
# `SPEED_OF_LIGHT` in metres per second from PositionVelocityTime, and `SECONDS_PER_WEEK`
# from GNSSDecoder.
...
rinex_week(
    decoder::GNSSDecoderState{
        <:Union{GNSSDecoder.GalileoINAVData,GNSSDecoder.GalileoE5aData},
    };
    approximate_year,
...
function rinex_ephemeris(
    ...
    GPSEphemeris(;
        ...
        omegadot = data.Ω_dot,
        idot = data.i_dot,
        codes_on_l2 = something(data.code_on_L2, 0),
        week = week,
        l2p_data_flag = something(data.L2_P_data_flag, false) ? 1.0 : 0.0,
        sv_accuracy = ura_metres(data.URA_index),
...
# GPS user range accuracy: the broadcast 4-bit URA index maps onto metres in two geometric
# segments (IS-GPS-200N 20.3.3.3.1.3, Table 20-I). GNSSDecoder reports the raw index — the
# table gives a *bound* rather than a value, and index 15 has no metre value at all — while
# the RINEX record's `SV accuracy` field is in metres, so the conversion happens here.
# Index 15 is "no accuracy prediction available", which RINEX reports as a negative
# accuracy, the same way `sisa_metres` reports Galileo's NAPA below.
function ura_metres(index::Integer)
    index <= 6 && return 2.0^(1 + index / 2)
    index <= 14 && return 2.0^(index - 2)
    -1.0
end
ura_metres(::Nothing) = -1.0
...
galileo_health(data::GNSSDecoder.GalileoINAVData) = galileo_sv_health(;
    e1b_dvs = data_validity_bits(data.E1B_DVS),
    e1b_hs = signal_health_bits(data.E1B_SHS),
    e5b_dvs = data_validity_bits(data.E5b_DVS),
    e5b_hs = signal_health_bits(data.E5b_SHS),
)
galileo_health(data::GNSSDecoder.GalileoE5aData) = galileo_sv_health(;
    e5a_dvs = data_validity_bits(data.E5a_DVS),
    e5a_hs = signal_health_bits(data.E5a_SHS),
)

galileo_sources(::GNSSDecoder.GalileoINAVData) =
...
galileo_bgd_e5b_e1(data::GNSSDecoder.GalileoINAVData) = data.BGD_E1_E5b
galileo_bgd_e5b_e1(::GNSSDecoder.GalileoE5aData) = 0.0

galileo_sisa(data::GNSSDecoder.GalileoINAVData) = sisa_metres(data.SISA_E1_E5b)
...
function nav_time_system_corrections!(corrections, data::GNSSDecoder.GPSL1CAData)
    isnothing(data.A_0UTC) || push!(
        corrections,
        TimeSystemCorrection("GPUT", data.A_0UTC, data.A_1UTC, data.t_0t, data.WN_0t),
    )
    corrections
end
function nav_time_system_corrections!(corrections, data::GNSSDecoder.AbstractGalileoData)
    isnothing(data.A_0UTC) || push!(
        corrections,
        TimeSystemCorrection("GAUT", data.A_0UTC, data.A_1UTC, data.t_0t, data.WN_0t),
    )
