...
# All of the gateware's channels. Eight are plenty for a fix once the loops are
# closed, but a first scan can hand over several false alarms along with the
# real satellites, and with eight channels those blocked every real satellite
# for a minute (issue #107).
const N_HW_CHANNELS = 20
...
function main()
    ...
    # Built here, with the link's defaults, only so its counters can be read at
    # the end; `receive(sdr, …)` builds the same one when none is passed.
    link = HardwareCorrelatorLink(sdr; sampling_freq = FS, reference_signal = gpsl1)
    # The public hardware method: asynchronous acquisition, the processing task
    # on the interactive pool, and the delay-aware `NCOReferencedPLLAndDLL` at
    # the signal's 18 Hz reference bandwidth. The conventional loop cannot hold
    # the few milliseconds between a record and the NCO write it motivates at
    # that bandwidth, and lowering it to 12 Hz — what this example used to do —
    # made the loop four to five times noisier than the software receiver's.
    data = receive(
        sdr,
        gpsl1,
        FS;
